...
public class DragonListener implements Listener {
    ...
    private void spawnEgg(World world) {
        ...
        // Проверяем, что яйцо не будет спавниться в воздухе или в блоке
        if (configManager.isCheckForFreeSpace()) {
            if (eggLocation.getBlock().getType() != Material.AIR) {
                // Если блок не пустой, размещаем яйцо выше
                ...
                // Если мы достигли максимальной высоты и все еще не нашли свободное место,
                // идем вниз от начальной позиции
                if (eggLocation.getBlock().getType() != Material.AIR) {
                    eggLocation = eggLocation.subtract(0, 1, 0); // Возвращаемся к последней проверенной позиции
